...
For the four scripts include a short "how to use" section in comments in the file.

Additionally generate:

5. site_analysis.json
6. endpoint_map.json
7. selectors.json
8. series_catalog.json
...
Do not generate common.py.

Do not create shared utility modules.

All generated endpoint scripts must be self-contained and executable without importing project-specific local files.
...
Each endpoint script must be fully self-contained.

Shared utilities may be duplicated across scripts when necessary.

Do not generate or reference common.py.

Avoid unnecessary code duplication where practical, but ensure every script can run independently.
...
Make sure to test these scripts to see if they actually work.
